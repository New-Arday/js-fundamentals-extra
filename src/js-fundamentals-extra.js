...
//
// TODO: write code below
function CheckTransactions(transactionsArr, startingBalance, accountOverdraft) {
  for (let i of transactionsArr) {
    startingBalance += i;
  }
  const finalBalance = startingBalance + accountOverdraft;
  if (finalBalance < 0) {
    return false;
  } else {
    return true;
  }
  // return startingBalance >= accountOverdraft;
}
...
function FilmsInGenre(filmName, filmGenre) {
  ...
  for (let i = 0; i < filmName.length; i++) {
    ...
    // console.log(filmName[0].genres.includes(filmGenre));
...
// console.log(FilmsInGenre(films, "Sci-Fi"));
